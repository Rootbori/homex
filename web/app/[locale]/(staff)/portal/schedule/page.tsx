...
export default async function SchedulePage({
  ...
  const [days, t] = await Promise.all([getSchedule(), Promise.resolve(getStaffDictionary(locale))]);

  return (
    <div className="mx-auto max-w-3xl">
      <header className="sticky top-0 z-50 border-b border-black/[0.04] bg-white/80 backdrop-blur-xl">
        <div className="flex h-14 items-center px-4">
          <h1 className="text-base font-bold text-on-surface">{t.schedule.title}</h1>
        </div>
      </header>
          ...
          days.map((day) => (
            <section key={day.date} className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-on-surface-variant/30">{formatStaffText(t.schedule.jobCount, { count: day.jobs.length })}</span>
              </div>
              {day.jobs.length > 0 ? (
                ...
                <p className="text-sm text-on-surface-variant/30">{t.schedule.noJobs}</p>
              )}
            </section>
          ))
        ) : (
