    ...
    <div className="mx-auto max-w-3xl">
      <header className="sticky top-0 z-50 border-b border-black/[0.04] bg-white/80 backdrop-blur-xl">
        <div className="flex h-14 items-center gap-3 px-4">
          ...
          <h1 className="text-base font-bold text-on-surface">{t.users.title}</h1>
      ...
      <main className="px-4 py-6 space-y-6">
        <div className="rounded-2xl bg-surface-container-low p-4 text-sm leading-6 text-on-surface-variant">
          {formatStaffText(t.users.scope, { store: store?.name ?? t.common.currentStoreFallback })}
        </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="rounded-xl bg-surface-container-low p-3 text-center">
            ...
            <p className="text-[10px] font-medium text-on-surface-variant/40">{t.common.customer}</p>
          </div>
          <div className="rounded-xl bg-surface-container-low p-3 text-center">
            <p className="text-lg font-extrabold text-on-surface">{formatCurrency(totalSpend)}</p>
            <p className="text-[10px] font-medium text-on-surface-variant/40">{t.users.totalSpend}</p>
          </div>
          <div className="rounded-xl bg-surface-container-low p-3 text-center">
            <p className="text-lg font-extrabold text-on-surface">
              {formatCurrency(users.length > 0 ? Math.round(totalSpend / users.length) : 0)}
            </p>
            <p className="text-[10px] font-medium text-on-surface-variant/40">{t.users.averagePerUser}</p>
          </div>
        </div>
        ...
        <div className="space-y-2">
          {users.length > 0 ? (
            users.map((user) => (
              <Link
                key={user.id}
                href={localizeAppPath(`/portal/users/${user.id}`, locale)}
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 text-xs text-on-surface-variant/40">
                    {user.phone} • {user.area} • {user.jobsCount} {t.common.jobs}
                  </p>
                  {user.note && (
