...
export default async function DashboardPage({
  ...
  const [dashboard, t] = await Promise.all([getDashboard(), Promise.resolve(getStaffDictionary(locale))]);
  const hour = new Date().getHours();
  
  let greeting = t.dashboard.morning;
  if (hour >= 12 && hour < 18) {
    ...
    <div className="mx-auto max-w-3xl">
      ...
      <header className="sticky top-0 z-50 border-b border-black/[0.04] bg-white/80 backdrop-blur-xl">
        <div className="flex h-14 items-center gap-3 px-4">
          ...
          <span className="text-base font-bold text-on-surface">{t.common.portalTitle}</span>
        </div>
      </header>
      ...
      <main className="px-4 py-6 space-y-6">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-on-surface-variant/50">{t.dashboard.summary}</p>
        </div>
        ...
        {dashboard.urgentJobs.length > 0 && (
          <section className="space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-bold text-on-surface">{t.dashboard.urgentJobs}</h2>
              ...
              {dashboard.urgentJobs.map((item) => (
                ...
                >
                  ...
                  <p className="mt-0.5 text-sm text-on-surface-variant/60">
                    {t.dashboard.customerPrefix}{item.user} — {item.status}
                  </p>
                </div>
              ))}
            </div>
        ...
        <section className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold text-on-surface">{t.dashboard.todayJobs}</h2>
            <Link href={localizeAppPath("/portal/jobs", locale)} className="flex items-center gap-1 text-xs font-semibold text-primary">
        ...
        <section className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold text-on-surface">{t.dashboard.latestLeads}</h2>
            <Link href={localizeAppPath("/portal/leads", locale)} className="flex items-center gap-1 text-xs font-semibold text-primary">
              {t.common.all} <ChevronRight className="h-3 w-3" />
            </Link>
          </div>
