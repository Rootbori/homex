...
export default async function MyRequestsPage() {
  const [jobs, locale] = await Promise.all([getUserJobs(), getPreferredLocale()]);

  return (
    <div className="mx-auto max-w-3xl">
      <header className="sticky top-0 z-50 border-b border-black/[0.04] bg-white/80 backdrop-blur-xl">
        <div className="flex h-14 items-center gap-3 px-4">
          <Link href={localizeAppPath("/", locale)} className="text-on-surface-variant/50 hover:text-on-surface">
        ...
        {jobs.length > 0 ? (
          <div className="space-y-3">
            {jobs.map((job) => (
              <Link
                key={job.id}
                href={localizeAppPath(`/tracking/${job.id}`, locale)}
          ...
          <div className="flex flex-col items-center py-20 text-center">
            ...
            <Link
              href={localizeAppPath("/request", locale)}
