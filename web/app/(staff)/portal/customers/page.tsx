...
export default async function CustomersPage() {
  const [session, customers] = await Promise.all([auth(), getCustomers()]);
  const totalSpend = customers.reduce((sum, customer) => sum + customer.totalSpend, 0);

  return (
    <div>
      <TopAppBar title="Customers" right={<ProfileBubble image={session?.user?.image ?? undefined} />} />
      <main className="page-content page-stack">
        ...
        <section className="grid gap-3 sm:grid-cols-3">
          <div className="rounded-[1.5rem] bg-primary-container p-4 text-on-primary">
            ...
            <p className="mt-2 text-3xl font-extrabold">{customers.length}</p>
          </div>
          ...
          <div className="rounded-[1.5rem] bg-surface-container-low p-4">
            ...
            <p className="mt-2 text-2xl font-extrabold text-on-surface">
              {formatCurrency(customers.length > 0 ? Math.round(totalSpend / customers.length) : 0)}
            </p>
          </div>
        </section>
        ...
        <div className="card-stack">
          {customers.length > 0 ? (
            customers.map((customer) => (
              <div key={customer.id} className="surface-card rounded-[1.75rem] p-5 ambient-shadow md:p-6">
                <div className="section-stack-sm">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="headline-font text-lg font-bold text-on-surface">{customer.name}</p>
                      <p className="text-sm text-on-surface-variant">{customer.phone}</p>
                    </div>
                    <p className="text-sm font-semibold text-on-surface">{formatCurrency(customer.totalSpend)}</p>
                  </div>
