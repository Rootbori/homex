  ...
  return (
    <div className="space-y-0">
      {items.map((item, index) => {
        let circleClass = "border-2 border-[var(--outline-variant)] bg-transparent";
        if (item.done) {
          circleClass = "bg-primary";
        } else if (index === items.findIndex((entry) => !entry.done)) {
          circleClass = "bg-primary ring-4 ring-primary/20";
        }
